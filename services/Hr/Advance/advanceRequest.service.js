...
exports.createAdvanceRequest = async (data) => {
  const { userId, advanceTypeId, amount } = data;
  ...
  const advanceType = await advanceTypesModel
    .findById(advanceTypeId)
    .populate("policyId");

  if (!advanceType) {
    throw new Error("Advance type not found");
  }

  const salary = Number(staff.salary?.amount || 0);

  const maxAllowed =
    (salary * (advanceType.maxPercentageOfSalary || 100)) / 100;

  if (amount > maxAllowed) {
    throw new Error(`Requested amount exceeds max allowed (${maxAllowed})`);
  }

  const request = await AdvanceRequest.create(data);
...
exports.handleApprovalTransaction = async (
  ...
  try {
    ...
    const salarySnapshot = Number(employee?.salary?.amount || 0);
    // =========================
