...
const getPaymentHistory = async (req, res, next) => {
  ...
  const transactionsWithBalance = orderedTransactions.map((tx) => {
    ...
    if (tx.entryType === "payment") {
      if (tx.actionType === "cancel") {
        // subtract the cancelled payment from totals
        payments -= amount;
        if (tx.balanceEffectType === "Withdrawal") totalOut -= amount;
        if (tx.balanceEffectType === "Deposit") totalIn -= amount;
      } else {
        payments += amount;
        if (tx.balanceEffectType === "Deposit") totalOut += amount;
        if (tx.balanceEffectType === "Withdrawal") totalIn += amount;
      }
    }
