  ...
  if (req.body.paymentsStatus === "paid") {
    ...
    payment = await paymentModel.create(
      [
        {
          source: {
            id: order[0].customer?.id || order[0]._id,
            name: order[0].customer?.name || order[0].invoiceName,
          },

          destination: {
            id: req.body.financailFund.id,
...
exports.calculateProfit = asyncHandler(async (req, res) => {
  const { invoiceDate, companyId, products } = req.body;

  if (!invoiceDate || !companyId || !products?.length) {
  ...
  const movements = await productMovementModel.aggregate([
    {
      $match: {
        ...
        companyId,
        createdAt: { $lte: new Date(invoiceDate) },
      },
    },
    { $sort: { createdAt: -1 } },
    {
  ...
  movements.forEach((m) => {
    ...
    const costInMainCurrency = rawEnterPrice / rate;

    costMap[m._id.toString()] = costInMainCurrency;
  });
  ...
  for (const item of products) {
    ...
    const selling = Number(item.sellingPrice); // already in main currency
    ...
    detailed.push({
      ...
      enterPrice: cost, // now in main currency
...
exports.editOrderInvoice = asyncHandler(async (req, res, next) => {
  ...
  req.body.invoicesItems.forEach((item) => {
    ...
    if (wasDraft && !isDraft) {
      ...
      const oldItem = originalItems.find((o) => o.id === item.id);
  ...
  try {
    ...
    if (req?.body?.paymentsStatus === "paid") {
      ...
      const payment = await paymentModel.create({
        ...
        destination: {
          id: order.customer?.id || order._id,
          name: order.customer?.name || order.invoiceName,
        },
        ...
        counter: String(conuter + nextCounterPayment),
