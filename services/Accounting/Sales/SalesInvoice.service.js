...
exports.applySalesInventoryEffectsService = async ({
  ...
  const invoiceCurrency = newSalesInvoice?.currency;
  ...
  for (let index = 0; index < invoicesItem.length; index++) {
    ...
    if (item.type === "expense") {
        ...
        [
          {
            ...
            companyId,
            date: newSalesInvoice.orderDate,
          },
        ],
        { session },
      );
      ...
      await createProductMovement({
        ...
        companyId,
        outPrice: Number(item.orginalBuyingPrice || 0),
        outPriceMainCurrrency:
          Number(item.orginalBuyingPrice || 0) / (item.exchangeRate || 1),
        stockId: null,
        sellingPrice: getOriginalSellingPrice(item) || 0,
      ...
      for (const fm of fifoMovements) {
        ...
        await createProductMovement({
          ...
          outPriceMainCurrrency: fm.costBuyingPrice / (item.exchangeRate || 1),
          stockId: item.stock._id,
