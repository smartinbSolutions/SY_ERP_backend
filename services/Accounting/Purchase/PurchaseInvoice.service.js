...
exports.findAllPurchaseInvoicesService = async ({ req, companyId }) => {
  ...
  const purchaseInvoices = await PurchaseInvoicesModel.aggregate([
    { $match: query },
    { $sort: { date: -1 } },
    { $skip: skip },
    { $limit: pageSize },
    {
      $lookup: {
        from: "payments", // confirm this matches Payment's actual collection name
            ...
            $match: {
              $expr: {
                $and: [
                  ...
                  { $eq: ["$allocations.documentType", "purchase_invoice"] },
    ...
    {
      $lookup: {
        from: "users", // confirm the actual collection name behind the "employee" ref model
        localField: "employee",
        foreignField: "_id",
        as: "employee",
        pipeline: [
          { $project: { name: 1, profileImg: 1, email: 1, phone: 1 } },
        ],
      },
    },
    {
  ...
  for (const item of inventoryItems) {
    if (item.type === "unTracedproduct") {
        ...
        [
          {
            ...
            enterPrice: item.orginalBuyingPrice || item.convertedBuyingPrice,
            enterPriceMainCurrency:
              item.convertedBuyingPrice /
                newPurchaseInvoice.currency.exchangeRate ||
      ...
      await createProductMovement({
        ...
        companyId,
        enterPrice: getOriginalBuyingPrice(item) || 0,
        enterPriceMainCurrency:
          Number(item.convertedBuyingPrice || 0) / exchangeRate || 0,
        stockId: item?.stock?._id || null,
        buyingPrice: getOriginalBuyingPrice(item) || 0,
  ...
  for (const item of inventoryItems) {
    if (item.type === "product") {
      ...
      await createProductMovement({
        ...
        companyId,
        outPrice: getOriginalBuyingPrice(item) || 0,
        outPriceMainCurrrency:
          Number(item.convertedBuyingPrice || 0) / invoiceExchangeRate || 0,
        stockId: item.stock?._id,
        buyingPrice: getOriginalBuyingPrice(item) || 0,
        exchangeRate: item.exchangeRate,
        movementDate: new Date(),
